...
- **Reassembly buffer (unicast).** `newReassembly(session)` starts empty; the buffer grows by doubling as packets land at higher offsets (amortized O(N), at most log₂(N) reallocations per session). A sparse interval set tracks received byte ranges; its overhead is bounded by ≤ K entries between merges, negligible relative to the buffer itself. The protocol carries no total-payload-size field, so the receiver can't auto-complete — the user manually clicks Save when their progress display matches what they expect. Auto-complete and exact progress percentages will arrive with the M11 back-channel or M12 handshake.
